            ...
            for idx_op in range(len(arch_parameters[idx_ct][idx_edge])):
                ...
                if alpha_active[idx_ct][idx_edge, idx_op] > 0:
                    ...
                    for _ in range(repeat):
                        ...
                        # ####################
                        ntk_delta.append(
                            round((ntk_origin - ntk) / ntk_origin, precision))  # higher the more likely to be prunned
                    ...
                    with torch.no_grad():
                        ...
                        for _ in range(repeat):
                            ...
                            _linear_regions.append(round((_lr - _lr_2) / _lr,
                                                         precision))  # change of #Regions, lower the more likely to be prunned
                            lrc_model.clear()
...
def is_single_path(network):
    ...
    edge_active = torch.cat([(nn.functional.softmax(alpha, 1) > 0.01).float().sum(1) for alpha in arch_parameters],
                            dim=0)
...
def main(xargs):
    ...
    lrc_model = Linear_Region_Collector(input_size=(1000, 1, 3, 3), sample_batch=3, dataset=xargs.dataset,
                                        data_path=xargs.data_path, seed=xargs.rand_seed)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, choices=['cifar10', 'cifar100', 'ImageNet16-120', 'imagenet-1k'],
